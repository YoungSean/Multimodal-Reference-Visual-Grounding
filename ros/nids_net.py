...
class NIDS_PE:
    ...
    def __init__(self, template_features=None, use_adapter=False, adapter_path=None, use_class_token=True,
        ...
        for i in range(num_imgs):
            ...
            with torch.no_grad():
                if self.use_class_token:
                    ...
                    image_feature = get_features_PE_FFA(img, mask, self.descriptor_model, self.preprocess, self.model_name, device=self.device).float()
                if self.use_adapter:
...
class NIDS_CLIP:
    ...
    def __init__(self, model, processor, template_features=None, use_adapter=False, adapter_path=None, use_class_token=True,
        ...
        for i in range(num_imgs):
            ...
            with torch.no_grad():
                if self.use_class_token:
                    ...
                    #img = self.preprocess(img).unsqueeze(0).to(self.device)
                    image_feature = self.encoder.get_image_features(**inputs)
                else:
                    image_feature = get_features_PE_FFA(img, mask, self.descriptor_model, self.preprocess, self.model_name, device=self.device).float()
                if self.use_adapter:
                    #image_feature /= image_feature.norm(dim=-1, keepdim=True)
