...
# model_weight = 'PE-Core-L14-336-original_refer_weight_042325_temp_0.05_epoch_640_lr_0.001_bs_1024_vec_reduction_4'
model_weight = "PE-Core-L14-336-FFA_refer_weight_04292025_temp_0.05_epoch_640_lr_0.001_bs_1024_vec_reduction_4"
adapter_descriptors_path = f"adapted_obj_feats/{model_weight}.json"

# adapter_descriptors_path = './object_features/PE-Core-L14-336_FFA.json'
...
model = NIDS_PE(object_features, use_adapter=True, adapter_path=weight_adapter_path, use_class_token=False, gdino_threshold=0.4, class_labels=labels, model_name = "PE-Core-L14-336")
...
def evaluate_coco(gt_path, pred_path):
    ...
    for k, v in (('info', {}), ('licenses', [])):
        if k not in coco_gt.dataset:
            coco_gt.dataset[k] = v
    # if you changed cocoGt.dataset, rebuild indices (harmless if already built)
    coco_gt.createIndex()
...
pred_annotations_path = os.path.join("results","pe_nids_net_PE-Core-L14-336-original_refer_weight_042325_temp_0.05_epoch_640_lr_0.001_bs_1024_vec_reduction_4.json")
#os.path.join("results", f"pe_nids_net_adapted_FFA_{model_weight}.json")
